								...
								<view @tap="target('/pages/team/releaseRouter/signalRouterDetail?id='+item.id)">
									<view style="align-items: center;">
										<view class="place-left" v-if="row.sort == -1">
											...
											<image style="width:80upx ;height: 60upx;"
											src="../../../static/icons/youjiantou.png"></image>
										</view>
...
<script>
	import {queryRouterDetail,deleteRouter} from '@/api/router'	
			...
			deleteRouter(id){
				...
				uni.showModal({
				    ...
				    success: function (res) {
				        if (res.confirm) {
							deleteRouter({method:"delete",ids:id}).then(({ data })=>{
								if(data.status == 0){
									uni.showToast({
									    title: '删除成功',
									    duration: 2000
