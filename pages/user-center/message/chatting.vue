					...
					<block>
						...
						<view class="other" style="padding:8upx 0upx" v-if="row.sender_id !== UserInfo.id">
							...
							<view class="right">
								...
								<!-- 语音消息 -->
								<!-- <view v-if="row.rec_rets.type=='voice'" class="bubble voice" @tap="playVoice(row.msg)" :class="playMsgid == row.msg.id?'play':''">
									<view class="icon other-voice"></view>
									<view class="length">{{row.msg.content.length}}</view>
								</view> -->
								<!-- 图片消息 -->
								<!-- <view v-if="row.rec_rets.type=='img'" class="bubble img" @tap="showPic(row.msg)">
									<image :src="row.msg.content.url" :style="{'width': row.msg.content.w+'px','height': row.msg.content.h+'px'}"></image>
								</view> -->
							</view>
						</view>
...
<script>
    import { Created_Chatting, Receive_Chatting, Look_ChatHistory } from "@/api/chatting";
	export default {
		components: { },
		data() {
			return {
				...
				scrollAnimation:false,//在设置滚动条位置时使用动画过渡
		...
		// onBackPress(options) {
		// 	uni.reLaunch({
		// 		   url: '/pages/user-center/message/mymessage?id=' + this.groupid + '&mumbers=' + this.mumbers + '&name=' + this.groupname
		// 	   });
  //   }
